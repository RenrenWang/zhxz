...
export default class OpenD extends React.Component {
   ...
   toComment(id){ 
    ...
    render() {
     ...
     <View
             style={{flexDirection:'column',height:this.props.itemHeight?this.props.itemHeight:210,borderBottomColor:"#dcdcdc"}}
             >
             <TouchableOpacity >
                   <Image  style={{width,height:210-45}} source={{uri:"http://121.40.241.28:7070/zhxz/"+data.villAnnex}}/>
             </TouchableOpacity>
             <View style={{height:45,flexDirection:'row',justifyContent:'space-between',flex:1,backgroundColor:'#fff',paddingHorizontal:10,alignItems:'center'}}>
                  <Text style={{fontSize:16,color:"#000"}}>{data.villTitle}</Text>
                  <View style={{flexDirection:'row'}}>
                      <Text style={{fontSize:16,color:'#999'}}>{data.villSeat}</Text>
 ...
 <View style={{marginTop:15,padding:10,backgroundColor:'#fff'}}>
                    <Text>{data.villDescription}</Text>
              </View>   
